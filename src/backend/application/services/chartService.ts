import { StoredHealthEntry } from '../../infrastructure/database/connection';
...
/**
 * Application service for chart generation
 */
export class ChartService {
  ...
  async generateChart(
    ...
    const weights = filteredData.map(entry => 
      this.conversionService.convertWeight(entry.weight, 'kg', weightUnit)
    );
    const waists = filteredData.map(entry => 
      this.conversionService.convertWaist(entry.waist, 'cm', waistUnit)
    );
